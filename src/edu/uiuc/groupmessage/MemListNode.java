...
class MemListNode {
  ...
  public void handleResetMemberList(List< Member > member_list) {
    System.out.println("Reset the member list");
    setMemberList(member_list);
    startHeartbeatClient();
    startFailureDetector();
    System.out.println(member_list.toString());
  }

  public GroupMessage handleJoinRequest(Member joiner) {
    // Log the event
    System.out.println(
      "The member " + memberToID(joiner) + " just joins.");
  ...
  public void handleTargetJoins(Member joiner) {
    ...
    if (!joiner.equals(currentMember)) {
      ...
      System.out.println("Duplicated Member Joins");
    }
  ...
  public void handleTargetLeaves(Member leaver) {
    // Log the event
    System.out.println(
      "The member " + memberToID(leaver) + " just leaves.");

    // Update the memberList
    if (!removeMember(leaver)) {
      System.out.println("Somebody leaves without being on the list");
    }
  ...
  public void handleTargetFails(Member loser) {
    // Log the event
    System.out.println(
      "The member " + memberToID(loser) + " just fails.");

    // Update the memberList
    if (!removeMember(loser)) {
      System.out.println("Somebody fails but he/she is not on the list");
    }
  ...
  public void sendMessageTo(GroupMessage msg, Member receiver) {
	...
	System.out.println(
	  "Finished Sending request to " + memberToID(receiver));
      }
  ...
  public void startHeartbeatClient() {
    ...
    System.out.println("Start heartbeating to " + memberToID(heartbeatTo));
  ...
  public void detectFailure() {
    ...
    if (current_time - getHeartbeatTimestamp() > 4500) {
      System.out.println("Detect failure.");
      stopFailureDetector();
